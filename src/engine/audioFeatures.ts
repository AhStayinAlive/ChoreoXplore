...
export async function attachAudio(el: HTMLMediaElement | MediaStream){
  ...
  const analyser=Meyda.createMeydaAnalyzer({
    audioContext:ctx, source:src, bufferSize:1024,
    ...
    callback:({rms,spectralCentroid})=>{
      ...
      raf=requestAnimationFrame(()=>{
        ...
        audio$.next({
          rms:rms??0, energy, centroid:spectralCentroid??0,
          bpmish:prev.bpmish*0.98+((energy>prev.energy)?1:0)*0.02
        });
      });
    }
  });
  src.connect(ctx.destination); analyser.start();
  return ()=>{ try{analyser.stop()}catch{} if(raf)cancelAnimationFrame(raf); try{ctx.close()}catch{} };
}
